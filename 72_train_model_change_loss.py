...
constraint_columns = ['HR', 'AR', 'NPP', 'COL_FIRE_CLOSS']
constraint_data = df[constraint_columns].copy()
...
constraint_data_tensor = torch.tensor(constraint_data.values, dtype=torch.float32)
...
for epoch in range(num_epochs):
    ...
    for i in range(0, train_size, batch_size):
        ...
        train_constraints_batch = train_constraints[i:end_idx]
        ...
        NPP = output[:, 9]  
        HR = output[:, 7]  
        COL_FIRE_CLOSS = output[:, 10]  
        AR = output[:, 8]
        constraint_1 = torch.mean((NPP - HR - COL_FIRE_CLOSS) ** 2)
        constraint_2 = torch.mean((NPP - (GPP - AR)) ** 2)

        loss = 0.8*mse_loss + 0.2 *( constraint_1 +  constraint_2)
